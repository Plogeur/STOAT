...
auto process_genotype_table(
    ...
    std::vector<PhenotypeType> phenotype_filtered;
    if constexpr (!std::is_same_v<PhenotypeType, void>) {
    ...
    if constexpr (std::is_same_v<PhenotypeType, void>) {
        return std::make_tuple(genotypes_filtered, index_used, allele_number, allele_paths);
    } else {
        return std::make_tuple(genotypes_filtered, phenotype_filtered, allele_number, allele_paths);
    }
}
...
create_eqtl_table(
    ...
    return process_genotype_table<std::string>(length_sample, column_headers, matrix);
}
