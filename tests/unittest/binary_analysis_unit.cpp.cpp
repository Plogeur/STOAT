...
TEST_CASE("Chi-square & Fisher test function", "[chi2Test]") {
    SECTION("Valid chi-square test & not valid Fisher test calculation") {
        ...
        REQUIRE(chi2Test(observed, 1).first != "NA");
        REQUIRE(fastFishersExactTest(observed, 1).first == "NA");
    }
    ...
    SECTION("Chi-square fail & Fisher test valid (zero row)") {
        ...
        REQUIRE(chi2Test(zero_row_matrix, 1).first == "NA");
        REQUIRE(fastFishersExactTest(zero_row_matrix, 1).first != "NA");
    }
}
