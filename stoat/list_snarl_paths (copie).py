...
def save_snarls(stree, root, pg, reference, pp_overlay) :
    ...
    def save_snarl_tree_node(net):
        if stree.is_snarl(net):
            ...
            path_handle = pg.get_path_handle_of_step(node_handle_t)
            path_name = pg.get_path_name(path_handle)
            position = str(pp_overlay.get_step_position(node_handle_t))
            if path_name in reference :
                save_pos_chr = (path_name, position)
            snarls.append({net : save_pos_chr})
